...
Official submission deadline: **August 31, 2026 at 5:00 PM Pacific Time**.

## Already complete

- Public GitHub repository with license, architecture, setup, tests, and generated evidence.
- Gemini 3.7 Flash through Vertex AI.
- Google ADK with a model-directed, user-defined expert panel.
- Cloud Run private service, background Job, and isolated public event gateway.
- Firestore fact ledger and Secret Manager.
- Real GitHub event → Job → expert events → GitHub packet proof.
...
- Record and upload the four-minute video using `docs/DEMO_SCRIPT.md` and
...
Do not expose Cloud credentials, API keys, Secret Manager values, or the private model-service URL
in the recording. Do not click Cloud Billing **Activate** or **Upgrade**.
